...
const ShowtimeWindow = () => {
  const { showWindow, selectedMovie, closeShowWindow, selectedTheater, theaters, onSelectTheater } = useShowtime(); // Access 'theaters' from context
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-3xl w-full h-3/4 overflow-auto">
        ...
        <h2 className="text-2xl font-bold mb-4 text-black">Showtimes for {selectedMovie.title}</h2>

        <div className="mb-6">
          <label className="block text-lg font-semibold mb-2 text-black">Select Theater:</label>
          <select
        ...
        <div>
          <h3 className="text-xl font-bold mb-4 text-black">Available Showtimes:</h3>
          {/* Replace this with real showtimes */}
          <ul>
            {["10:00 AM", "1:00 PM", "5:30 PM", "8:45 PM"].map((time, index) => (
